...
export async function GET(request) {
  ...
  const headers = { "Content-Type": "application/pdf", "Content-Disposition": `${disposition}; filename=sedanur-ceylan-cv.pdf`, "Cache-Control": "public, max-age=300" };
  try {
  ...
  const fallback = await readFile(path.join(process.cwd(), "public", "cv.pdf"));
  return new NextResponse(fallback, { headers });
}
